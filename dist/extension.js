...
// node_modules/@thecollege/azure-test-track/lib/devops.js
var require_devops = __commonJS({
  "node_modules/@thecollege/azure-test-track/lib/devops.js"(exports2, module2) {
    var axios = require_axios();
    var organization = process.env.ADO_ORGANIZATION;
    var project = process.env.ADO_PROJECT;
    var personalAccessToken = process.env.ADO_PERSONAL_ACCESS_TOKEN;
    var companyEmail = process.env.ADO_COMPANY_EMAIL;
    var headers = {
      "Authorization": `Basic ${Buffer.from(":" + personalAccessToken).toString("base64")}`
    };
    var getPlanIdByName = async (planName) => {
    ...
    var getSuitesByPlanId = async (planId) => {
    ...
    var getTestPointByTestCaseId = async (planId, suiteId2, testCaseId) => {
      const pointsUrl = `https://dev.azure.com/${organization}/${project}/_apis/test/Plans/${planId}/Suites/${suiteId2}/points?testCaseId=${testCaseId}&api-version=7.1`;
      try {
    ...
    var getAllTestPointsByPlanAndSuite = async (planId, suiteId2) => {
      const url = `https://dev.azure.com/${organization}/${project}/_apis/test/Plans/${planId}/Suites/${suiteId2}/points?api-version=7.1`;
      try {
        const response = await axios.get(url, { headers });
        ...
        console.error(`Error to get Test Points from suite ${suiteId2}:`, error.response?.data || error.message);
        return [];
      }
    };
    var getTestPointsData = async (planId, resultData) => {
      if (!planId) return [];
      ...
      const filteredTestPointsData = allTestPoints.filter((testPoint) => testCaseIdsSet.has(testPoint.testCase.id.toString())).map((testPoint) => ({
    ...
    var getTestPointIdsFromTestCases = async (planName, testCaseIds) => {
    ...
    var getAllTestPointsByPlanName = async (planName) => {
    ...
    var createTestRun = async (runSettings) => {
    ...
    var createTestRunWithoutTests = async () => {
      const url = `https://dev.azure.com/${organization}/${project}/_apis/test/runs?api-version=7.1`;
      const data = {
    ...
    var getTestResultsFromTestRun = async (runId) => {
    ...
    var updateTestRunResults = async (runId, newResultsData) => {
    ...
    var addTestResults = async (runId, resultData) => {
      const url = `https://dev.azure.com/${organization}/${project}/_apis/test/runs/${runId}/results?api-version=7.1`;
    ...
    var completeTestRun = async (runId) => {
    ...
    var resetAllTestPointsToActiveByPlanId = async (planId, testPoints) => {
      for (const testPoint of testPoints) {
        const suiteId2 = testPoint.suiteId;
        const testPointId = testPoint.testPointId;
        const updateUrl = `https://dev.azure.com/${organization}/${project}/_apis/test/Plans/${planId}/Suites/${suiteId2}/points/${testPointId}?api-version=7.1`;
        try {
          await axios.patch(
            updateUrl,
            { resetToActive: true },
          ...
          console.log(`Test point ${testPointId} in suite ${suiteId2} reset to Active.`);
        } catch (error) {
          console.error(`Error resetting test point ${testPointId} in suite ${suiteId2}:`, error.response?.data || error.message);
        }
      }
    };
    var getTestRunsByBuildId = async (buildId) => {
    ...
    var getWorkItemById2 = async (workItemId) => {
      const url = `https://dev.azure.com/${organization}/${project}/_apis/wit/workitems?ids=${workItemId}&api-version=7.1`;
      try {
        const response = await axios.get(url, { headers });
    ...
    var associtedTestCaseToAutomation2 = async (testCaseId, automatedTestCaseName, automationTestType) => {
      ...
      try {
        ...
        const response = await axios.patch(url, body, { headers: headers2 });
    ...
    module2.exports = {
      ...
      associtedTestCaseToAutomation: associtedTestCaseToAutomation2,
      getWorkItemById: getWorkItemById2
    };
  }
});
...
// node_modules/@thecollege/azure-test-track/extractor/extractor-test-results.js
var require_extractor_test_results = __commonJS({
  "node_modules/@thecollege/azure-test-track/extractor/extractor-test-results.js"(exports2, module2) {
    var fs = require("fs");
      ...
      return new Promise((resolve, reject) => {
        fs.readFile(filePath, "utf-8", (err, data) => {
          ...
          xml2js.parseString(data, (err2, result) => {
            if (err2) {
              console.error("Error parsing XML:", err2);
              return reject(err2);
            }
            if (!result || !result.testsuites || !result.testsuites.testsuite) {
              return resolve([]);
            }
            const newResultsData = result.testsuites.testsuite.flatMap(
              (suite) => {
          ...
          try {
            ...
            const results = parsedData.suites.flatMap((suite) => {
              return suite.specs.map((spec) => {
                ...
                const testCaseId = testCaseIdMatch ? parseInt(testCaseIdMatch[1]) : null;
                const outcome = spec.tests.every(
                  (test) => test.results.every((result) => result.status === "passed")
                ) ? "Passed" : "Failed";
                return testCaseId ? { testCaseId, outcome } : null;
              });
            }).filter(Boolean);
    ...
    module2.exports = { readAndProcessJUnitXML, readAndProcessCucumberJSON, readAndProcessPlaywrightJSON };
  }
});

// node_modules/@thecollege/azure-test-track/lib/reporter.js
var require_reporter = __commonJS({
  "node_modules/@thecollege/azure-test-track/lib/reporter.js"(exports2, module2) {
    var devops = require_devops();
    var extractor = require_extractor_test_results();
    var createTestRunByExecution = async (testSettings) => {
      let testResults;
...
// src/commands/extractors.ts
var vscode = __toESM(require("vscode"));
var path = __toESM(require("path"));
function extractTestCaseName(editor, startLine) {
  let testCaseName = "";
  let line = startLine;
...
function extractTestCaseNamesFromDocument(editor) {
  const testCaseNames = [];
  let line = 0;
  const language = editor.document.languageId;
  while (line < editor.document.lineCount) {
...
function extractTestCaseIds(editor, line) {
  const document2 = editor.document;
  let testCaseIds = null;
  const language = editor.document.languageId;
  const currentLineText = document2.lineAt(line - 1).text;
  ...
  if (!testCaseIds) {
    ...
    for (let i = line - 2; i >= startLine; i--) {
      const text = document2.lineAt(i).text.trim();
      if (language === "python") {
      ...
      const match2 = text.match(/ADO_IDs:\s*((?:TC_\d+,?\s*)+)/);
      if (match2) {
        const ids = match2[0].match(/TC_(\d+)/g);
        if (ids) {
          testCaseIds = ids.map((id) => id.replace("TC_", "").trim());
          break;
        }
...
function applyRelativePathToTestCaseName(editor, testCaseName) {
  if (testCaseName) {
...
// src/commands/commands.ts
var import_azure_test_track2 = __toESM(require_reporter());

// src/commands/association.ts
var vscode3 = __toESM(require("vscode"));
var import_azure_test_track = __toESM(require_reporter());
...
function checkEnvironmentVariables() {
  ...
  if (!organization || !project || !token || !email) {
    vscode2.window.showErrorMessage("Environment variables are not configured correctly.");
    return;
  }
}

// src/commands/association.ts
async function associateTestCase(testCaseId, testCaseName, testCaseType) {
  try {
    checkEnvironmentVariables();
    await (0, import_azure_test_track.associtedTestCaseToAutomation)(
      testCaseId,
      testCaseName,
      testCaseType
    );
    vscode3.window.showInformationMessage(`Test successfully associated with test case ${testCaseId}!`);
  } catch (error) {
    if (error instanceof Error) {
      vscode3.window.showErrorMessage(`Error associating test: ${error.message}`);
    } else {
      vscode3.window.showErrorMessage("Error associating test: unknown error.");
    }
  }
}

// src/decorators/association-decoretor.ts
var vscode4 = __toESM(require("vscode"));
var associatedDecoration = vscode4.window.createTextEditorDecorationType({
...
var unassociatedDecoration = vscode4.window.createTextEditorDecorationType({
...
// src/commands/commands.ts
var testTypes = ["Unit", "Component", "API", "E2E"];
var cache = /* @__PURE__ */ new Map();
async function associateTestCaseCommand(editor) {
  const position = editor.selection.active;
  let testCaseName = extractTestCaseName(editor, position.line);
  if (!testCaseName) {
    vscode5.window.showErrorMessage("Unable to extract test case name.");
    return;
  }
  const testCaseId = await vscode5.window.showInputBox({ prompt: "Enter the Test Case ID in Azure DevOps" });
  if (testCaseId === void 0) {
    return;
  }
  const testCaseType = await vscode5.window.showQuickPick(testTypes, { placeHolder: "Choose the Test Type" });
  if (testCaseType === void 0) {
    return;
  }
  if (!testCaseId || !testCaseName || !testCaseType) {
    vscode5.window.showErrorMessage("Incomplete information. Association canceled.");
    return;
  }
  await associateTestCase(testCaseId, testCaseName, testCaseType);
}
async function associateTestCaseCustomCommand() {
  const testCaseId = await vscode5.window.showInputBox({ prompt: "Enter the Test Case ID in Azure DevOps" });
  if (testCaseId === void 0) {
    return;
  }
  const testCaseName = await vscode5.window.showInputBox({ prompt: "Enter the Test Case Name in Azure DevOps" });
  if (testCaseId === void 0) {
    return;
  }
  const testCaseType = await vscode5.window.showQuickPick(testTypes, { placeHolder: "Choose the Test Type" });
...
async function associateTestCaseIdsFromComments(editor) {
  const testCaseNames = extractTestCaseNamesFromDocument(editor);
  if (testCaseNames.length === 0) {
    vscode5.window.showErrorMessage("No test cases found in the file.");
    return;
  }
  ...
  for (const { testCaseName, lineNumber } of testCaseNames) {
    ...
    if (!testCaseIds) {
      vscode5.window.showErrorMessage(`No ADO_ID found above the test case '${testCaseName}'.`);
      continue;
    }
    for (const testCaseId of testCaseIds) {
      await associateTestCase(testCaseId, applyRelativePathToTestCaseName(editor, testCaseName), testCaseType);
    }
    vscode5.window.showInformationMessage(`IDs successfully associated with the test case '${testCaseName}'.`);
  }
}
async function setDecorationsForAssociatedAutomatedTests(editor) {
  const testCaseNames = extractTestCaseNamesFromDocument(editor);
  if (testCaseNames.length === 0) {
    vscode5.window.showErrorMessage("No test cases found in the file.");
    return;
  ...
  const unassociatedLines = [];
  const processedTestCaseNames = /* @__PURE__ */ new Set();
  for (const { testCaseName, lineNumber } of testCaseNames) {
    ...
    const statusPromises = testCaseIds.map(async (testCaseId) => {
      checkEnvironmentVariables();
      const workItemInfo = await (0, import_azure_test_track2.getWorkItemById)(testCaseId);
      ...
      if (existingTestCase && existingTestCase.isAssociated) {
        associatedLines.push(new vscode5.Range(lineNumber, 0, lineNumber, 0));
      } else {
        unassociatedLines.push(new vscode5.Range(lineNumber, 0, lineNumber, 0));
      }
    });
    await Promise.all(statusPromises);
  }
  if (associatedLines.length > 0) {
    const consolidatedDecorations = /* @__PURE__ */ new Map();
    ...
    const decorations = Array.from(consolidatedDecorations.entries()).map(([lineNumber, idsSet]) => {
      ...
      return {
        range: new vscode5.Range(lineNumber, 0, lineNumber, 0),
        renderOptions: {
  ...
  if (unassociatedLines.length > 0) {
    const consolidatedUnassociatedDecorations = /* @__PURE__ */ new Map();
...
// src/extension.ts
var decoratedEditors = /* @__PURE__ */ new Map();
function getEditor() {
  const editor = vscode6.window.activeTextEditor;
  if (!editor) {
    vscode6.window.showErrorMessage("No file is open.");
    return;
  }
  return editor;
}
function activate(context) {
  const associateTestCommand = vscode6.commands.registerCommand("extension.associateTestCase", async () => {
    const editor = getEditor();
    await associateTestCaseCommand(editor);
  });
  const associateTestCommandCustom = vscode6.commands.registerCommand("extension.associateTestCaseCustom", async () => {
    await associateTestCaseCustomCommand();
  });
  const associateTestCaseIdsCommand = vscode6.commands.registerCommand("extension.associateTestCaseIdsFromComments", async () => {
    const editor = getEditor();
    await associateTestCaseIdsFromComments(editor);
  });
  const setDecorationsForAssociatedAutomatedTestsCommand = vscode6.commands.registerCommand("extension.setDecorationsForAssociatedAutomatedTests", async () => {
    const editor = getEditor();
    if (!editor) {
      return;
    }
  ...
  vscode6.window.onDidChangeTextEditorSelection((e) => {
    ...
    if (editor) {
      ...
      vscode6.commands.executeCommand("setContext", contextKey, isTestLine);
      console.log({
        lineText,
        language,
        isTestLine
